...
switch ($_GET['t']) {
    // Construye un tag html <select> con las series de contadores de una sucursal. Se usa en contratos.
    case 'contadorContratos':
        $tag = '<div class="Item" id="div_Contratos_IDContador">' . getContadoresSucursal($_GET['filtro'], 0, 'Contratos[IDContador]', 'Contratos_IDContador') . '</div>';
        break;

    // Construye un tag html <select> con los comerciales de una sucursal. Se usa en contratos.
    case 'comercialContratos':
        $tag = '<div class="Item" id="div_Contratos_IDComercial">' . getComercialesSucursal($_GET['filtro'], 'Contratos[IDComercial]', 'Contratos_IDComercial') . '</div>';
        break;
    ...
    case 'clientecontrato':
        //$tag  = '<div class="Item" id="div_Contrato_IDCliente">' . clientesSucursalAgente($_GET['filtro'],'', $_GET['nameselect'], $_GET['idselect']) . '</div>';
        $tag = '<div class="Item" id="div_Contratos_IDComercial">' . getComercial($_GET['filtro'], 'Contratos[IDComercial]', 'Contratos_IDComercial') . '</div>';
        //$tag .= '<div class="Item" id="div_Contrato_IDDirec">' . dEntregaCliente($_GET['filtro'], 'Contrato[IDDirec]', 'Contrato_IDDirec') . '</div>';
        $tag .= '<div class="Item" id="div_Contratos_IDFP">' . formasPagoCliente($_GET['filtro'], 'Contratos[IDFP]', 'Contratos_IDFP') . '</div>';
        break;
    ...
    case 'comercialcliente':
        ...
        $tag .= '<div class="Item" id="div_Contrato_IDDirec">' . dEntregaCliente($_GET['filtro'], 'Contrato[IDDirec]', 'Contrato_IDDirec') . '</div>';
        $tag .= '<div class="Item" id="div_Contrato_IDFP">' . formasPagoCliente($_GET['filtro'], 'Contrato[IDDirec]', 'Contrato_IDDirec') . '</div>';
        break;
...
function almacenesEmpresa($idEmpresa) {
    ...
    $rows = $almacen->fetchAll('Nombre');
    unset($almacen);

    $ch = "<div class='Etiqueta'>Almacén</div>";
...
function clientesComercial($idComercial, $nameSelect='', $idSelect='') {
    ...
    $ch .= "<select name='" . $nameSelect . "' id='" . $idSelect . "' class='Select'";
    $ch .= " onchange=\"DesplegableAjax('div_Contrato_IDDirec','Contrato_IDDirec','Contrato[IDDirec]','dentrega',this.value);";
    $ch .= "DesplegableAjax('div_Contrato_IDFP','Contrato_IDFP','Contrato[IDFP]','formaspago',this.value);\"";
    $ch .= ">";
